...
export default function LeftPanel({
  ...
  const overrides = assignmentOverrides ?? localOverrides;

  const setOverrides = (updater) => {
    if (typeof onChangeAssignmentOverrides === "function") {
      onChangeAssignmentOverrides((prev) => {
        const base = prev || {};
  ...
  const handleSave = async () => {
    ...
    const nextWorkerName = resolveWorkerName(workerId);
  ...
  const groups = useMemo(() => {
    if (Array.isArray(rows) && rows.length) {
      ...
      for (const r of rows) {
        if (r.type === "group") {
          const groupKey =
            r.workerKey ?? r.groupId ?? r.productId ?? String(r.key || "");
          current = {
            groupKey,
            groupName:
              r.workerName ??
              r.groupName ??
              r.productName ??
              (groupKey === "unassigned" ? "미배정" : String(groupKey)),
            open: Boolean(r.open ?? open?.[groupKey]),
            tasks: [],
          };
          out.push(current);
        } else if (r.type === "task") {
          if (!current) {
            const wk = r.workerId ? String(r.workerId) : "unassigned";
            current = {
              groupKey: wk,
              groupName: r.workerId ? r.workerName : "미배정",
              open: Boolean(open?.[wk] ?? true),
              tasks: [],
            };
            out.push(current);
          }
      ...
      return out.map((g) => ({
        ...g,
        open: Boolean(open?.[g.groupKey] ?? g.open),
      }));
    }
    ...
    for (const p of list) {
      ...
      for (let idx = 0; idx < schedules.length; idx++) {
        ...
        const wk = workerId ? String(workerId) : "unassigned";

        if (!map.has(wk)) {
          map.set(wk, {
            groupKey: wk,
            groupName: workerId ? workerName : "미배정",
            open: Boolean(open?.[wk] ?? true),
            tasks: [],
          });
          order.push(wk);
      ...
      >
        <div className="px-2">
          {groups.map((g) => {
            ...
            return (
              <div key={`group:${g.groupKey}`}>
                <div style={{ height: rowHeight }} className="flex items-center">
                  <button
                    onClick={() => onToggle?.(g.groupKey)}
                    className={[
                      "w-full h-full flex items-center justify-between rounded-xl px-3 transition",
                      isOpen
                        ? "bg-emerald-50 text-emerald-900"
                        : "bg-white text-slate-800 hover:bg-emerald-50/50",
                            ...
                            <div className="pl-6 pr-3 min-w-0 w-full">
                              ...
                              >
                                <PopoverTrigger asChild>
                                  <button
                                    type="button"
                                    className="w-full text-left rounded-lg px-2 py-1.5 border border-transparent hover:border-slate-200 hover:bg-slate-50 transition"
                                  ...
                                  >
                                    ...
                                    <div className="text-[11px] text-slate-500 truncate leading-4">
                                      {r.taskName}
                                    </div>
                                  </button>
                                </PopoverTrigger>
