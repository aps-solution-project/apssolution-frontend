...
export async function getMyChats(token) {
  const resp = await fetch(
    `http://${process.env.NEXT_PUBLIC_APS_SURVER_ADDRESS}:8080/api/chats`,
    {
      method: "GET",
...
export async function getChatDetail(token, chatId) {
  const resp = await fetch(
    `http://${process.env.NEXT_PUBLIC_APS_SURVER_ADDRESS}:8080/api/chats/${chatId}`,
    {
      method: "GET",
      headers: { Authorization: `Bearer ${token}` },
    },
...
export async function sendMessage(token, chatId, body) {
  ...
  const res = await fetch(
    `http://${process.env.NEXT_PUBLIC_APS_SURVER_ADDRESS}:8080/api/chats/${chatId}/message`,
    {
      method: "POST",
      headers: {
...
export async function startDirectChat(token, targetId) {
  const resp = await fetch(
    `http://${process.env.NEXT_PUBLIC_APS_SURVER_ADDRESS}:8080/api/chats/direct/${targetId}`,
    {
      method: "POST",
      headers: { Authorization: `Bearer ${token}` },
    },
...
export async function leaveChat(token, chatId) {
  const resp = await fetch(
    `http://${process.env.NEXT_PUBLIC_APS_SURVER_ADDRESS}:8080/api/chats/${chatId}/leave`,
    {
      method: "DELETE",
      headers: {
        Authorization: `Bearer ${token}`,
...
export async function getUnreadCount(token) {
  const resp = await fetch(
    `http://${process.env.NEXT_PUBLIC_APS_SURVER_ADDRESS}:8080/api/chats/unread-count`,
    {
      method: "GET",
      headers: {
        Authorization: `Bearer ${token}`,
